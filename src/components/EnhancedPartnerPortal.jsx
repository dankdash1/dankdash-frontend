...
import PartnerApplicationManager from './PartnerApplicationManager';
import { 
  Users, 
  TrendingUp, 
  DollarSign, 
...
const EnhancedPartnerPortal = ({ isAdmin = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Prepare application data for backend API
      const applicationData = {
        first_name: formData.firstName,
        last_name: formData.lastName,
        email: formData.email,
        phone: formData.phone,
        partner_type: selectedPartnership.id,
        business_name: formData.businessName || '',
        experience: formData.experience || '1-2',
        services: formData.services || [],
        vehicle_type: formData.vehicleType || '',
        availability: formData.availability || '',
        notifications: {
          email: formData.emailNotifications || true,
          sms: formData.smsNotifications || false,
          marketing: formData.marketingEmails || false
        }
      };

      // Submit to backend API
      const response = await fetch('https://5000-i0vjmutfi7k3dpzzbobaz-be7b87fb.manus.computer/api/partners/apply', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Reset form and go back to partner list
        setFormData({});
  ...
  const renderApplications = () => {
    // If admin user, show the application management interface
    if (isAdmin) {
      return <PartnerApplicationManager />;
    }
    
    // Otherwise show the partner application form interface
    return (
