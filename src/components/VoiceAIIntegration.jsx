...
import { Phone, Volume2, Play, Pause, Settings, Users, AlertCircle, CheckCircle, Mail, Send } from 'lucide-react';
...
const VoiceAIIntegration = () => {
  ...
  const BACKEND_URL = 'https://5000-i0vjmutfi7k3dpzzbobaz-be7b87fb.manus.computer';
  ...
  // SendGrid Configuration Functions
  const updateSendgridSettings = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`${BACKEND_URL}/api/sendgrid/config`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          api_key: sendgridSettings.apiKey,
          from_email: sendgridSettings.fromEmail,
          from_name: sendgridSettings.fromName
        })
      });

      const result = await response.json();
      if (result.success) {
        setSendgridSettings(prev => ({ ...prev, enabled: true }));
  ...
  const testSendgridConnection = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`${BACKEND_URL}/api/sendgrid/test`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          content: 'This is a test email from DankDash SendGrid integration.'
        })
      });
      ...
      if (result.success) {
        ...
        alert('SendGrid connection failed: ' + result.message);
      }
    } catch (error) {
      alert('Connection error: ' + error.message);
    } finally {
  ...
  const sendTestSendgridEmail = async () => {
    const testEmail = prompt('Enter email address to send test SendGrid email:');
    if (!testEmail) return;
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/sendgrid/test`, {
        ...
        body: JSON.stringify({
          to_email: testEmail,
          subject: 'DankDash SendGrid Test',
          content: 'Hello! This is a test email from your DankDash SendGrid integration. If you received this message, SendGrid is working correctly!'
        })
      });

      const result = await response.json();
      if (result.success) {
        alert('Test SendGrid email sent successfully!');
      } else {
        alert('SendGrid email failed: ' + result.message);
      }
    } catch (error) {
      alert('Email error: ' + error.message);
    } finally {
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSendgridSettings(prev => ({ ...prev, apiKey: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSendgridSettings(prev => ({ ...prev, fromEmail: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSendgridSettings(prev => ({ ...prev, fromName: e.target.value }))}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        checked={sendgridSettings.templates.orderConfirmation}
                        onChange={(e) => setSendgridSettings(prev => ({
                          ...prev,
                          templates: { ...prev.templates, orderConfirmation: e.target.checked }
                        }))}
                        className="mr-2"
                      />
                    ...
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        checked={sendgridSettings.templates.partnerWelcome}
                        onChange={(e) => setSendgridSettings(prev => ({
                          ...prev,
                          templates: { ...prev.templates, partnerWelcome: e.target.checked }
                        }))}
                        className="mr-2"
                      />
                    ...
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        checked={sendgridSettings.templates.applicationConfirmation}
                        onChange={(e) => setSendgridSettings(prev => ({
                          ...prev,
                          templates: { ...prev.templates, applicationConfirmation: e.target.checked }
                        }))}
                        className="mr-2"
                      />
                ...
                <div className="flex space-x-3">
                  <button
                    onClick={updateSendgridSettings}
                    disabled={isLoading}
                    className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 disabled:opacity-50"
                  >
                    {isLoading ? 'Updating...' : 'Update SendGrid Settings'}
                  </button>
                  <button
                    ...
                    className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50"
                  >
                    {isLoading ? 'Testing...' : 'Test SendGrid Connection'}
                  </button>
                  <button
                    onClick={sendTestSendgridEmail}
                    disabled={isLoading}
                    className="bg-orange-600 text-white px-4 py-2 rounded-lg hover:bg-orange-700 disabled:opacity-50"
                  >
                    {isLoading ? 'Sending...' : 'Send Test Email'}
                ...
                {sendgridSettings.enabled && (
                  <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center">
                      <CheckCircle className="h-5 w-5 text-green-500 mr-2" />
                      <span className="text-green-700 font-medium">SendGrid is configured and ready!</span>
                    </div>
                  </div>
                )}
              </div>
